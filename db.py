...
class Datebase:
    ...
    def mute(self, user_id, chat_id):
        with self.connection:
            user = self.cursor.execute("SELECT * FROM users WHERE user_id = ? AND chat_id = ?", (user_id, chat_id, )).fetchone()
            return int(user[2]) >= int(time.time())

    def add_mute(self, user_id, chat_id, mute_time):
        with self.connection:
            return self.cursor.execute("UPDATE users SET mute_time = ? WHERE user_id = ? AND chat_id = ?", (int(time.time()) +
